<?php
    ...
    switch($icon) {
        case 'email':
            $icon_code = '<svg class="link-icon" viewBox="0 0 14 10" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            break;
        case 'password':
            $icon_code = '<svg class="link-icon" viewBox="0 0 12 14" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    if (!function_exists('form_field')) {
        function form_field($p, $i = null) {
            ...
            // Set link icon for link field for link input

            if ($p['label'] === "Link") {
                $icon = '<svg class="link-icon" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            // If input type is an image, add additional html;
            ...
            if ($p['type'] === 'image') {
                ...
                $icon = '
                    <svg class="image-upload-icon" viewBox="0 0 34 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                $image_placeholder = '<h4>+ Upload Image</h4>';
            }
            ...
            foreach(LINK_OPTIONS as $index => $option) {
                ...
                $html_options .= <<<HTML
                    <li data-value="{$name_lowercase}" data-placeholder="{$option['placeholder']}" data-color="{$option['color']}" data-requiredtext="{$option['required_text']}">{$option['icon']}<span data-dropdownlistoption>{$option['name']}</span></li>
                HTML;
                ...
                if ($index === 0) {
                    ...
                    $selected_option = <<<HTML
                        {$option['icon']}
                        <span data-platformoptionselected data-value="{$name_lowercase}">{$option['name']}</span>
                        <svg class="platform-dropdown-arrow" viewBox="0 0 14 9" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<div class="form-field <?php echo $props['type'] === 'image' ? 'image-field' : ''; ?>" data-fieldparent data-fieldname="<?php echo $props['name'] ?? null; ?>" data-fieldtype="<?php echo $props['type']; ?>">
    <?php if ($props['type'] !== 'link'): ?>
        <?php echo form_field($props, $icon_code); ?>
    <?php else :?>
        <div class="link-number-remove-container">
            <div class="link-number-container" data-linkcollapseclick >
                <svg class="link-collapse-icon" viewBox="0 0 12 6" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="platform-link-fields-container">
            <?php 
